...
} from "@medusajs/medusa/dist/strategies/batch-jobs/product/utils";
...
import { productVariantCustomImportColumnsDefinition } from "./products/column-definition";
...
class ImportProductStrategy extends AbstractBatchJobStrategy {
		...
		this.csvParser_ = new CsvParser({
			columns: [
				...
				...(isProductCategoriesFeatureOn
					...
					: []),
				...productVariantCustomImportColumnsDefinition.columns,
			],
		});
	...
	protected static logDescriptiveError(
		row: TParsedProductImportRowData,
		errorDescription?: string
	): void {
		const message = `Error while processing row with
		...
		for (const productOp of productOps) {
			...
			try {
				...
				if (
					...
					const collection = await productCollectionServiceTx
						.retrieveByHandle(
					...
					if (!collection) {
						// create collecrion if it doesn't exist
						const collectionData = {
							title: productOp["product.collection.title"] as string,
							handle: productOp["product.collection.handle"] as string,
						};
						const createdCollection = await productCollectionServiceTx.create(
							collectionData
						);
						productData.collection_id = createdCollection.id;
					} else {
				...
				// ImportProductStrategy.throwDescriptiveError(productOp, e.message);
				ImportProductStrategy.logDescriptiveError(productOp, e.message);
			}
	...
	private async createVariants(batchJob: ProductImportBatchJob): Promise<void> {
		...
		for (const variantOp of variantOps) {
			try {
				...
				// ImportProductStrategy.throwDescriptiveError(variantOp, e.message);
				ImportProductStrategy.logDescriptiveError(variantOp, e.message);
			}
		}
	}
			...
			if (results[op]?.length) {
				const { writeStream, fileKey, promise } = await this.fileService_
					...
					.getUploadStreamDescriptor({
						name: ImportProductStrategy.buildFilename(batchJobId, op),
						ext: "json",
					});
...
export default ImportProductStrategy;
