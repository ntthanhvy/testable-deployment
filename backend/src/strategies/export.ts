...
import {
	BatchJobStatus,
	CreateBatchJobInput,
} from "@medusajs/medusa/dist/types/batch-job";
...
import { csvCellContentFormatter } from "@medusajs/medusa/dist/utils";
import { prepareListQuery } from "@medusajs/medusa/dist/utils/get-query-config";
import { productVariantCustomColumnsDefinition } from "./products/column-definition";
...
class ProductExportStrategy extends AbstractBatchJobStrategy {
	...
	protected readonly columnsDefinition = {
		...productColumnsDefinition,
		...productVariantCustomColumnsDefinition,
	};
	...
	public async buildHeader(batchJob: ProductExportBatchJob): Promise<string> {
		...
		this.appendProductCategoriesDescriptors(
			dynamicProductCategoriesColumnCount
		);
			...
			this.columnsDefinition[columnNameName] = {
				...
				exportDescriptor: {
					accessor: (productOption: Product) => {
						opt_id = productOption?.options[i]?.id ?? "";
						return productOption?.options[i]?.title ?? "";
					},
					entityName: "product",
				},
			};
			...
			this.columnsDefinition[columnNameNameValue] = {
				...
				exportDescriptor: {
					accessor: (variant: ProductVariant) => {
						return variant?.options.find((o) => o.option_id === opt_id)?.value ?? "";
					},
					entityName: "variant",
				},
			};
	...
	private buildProductVariantLines(product: Product): string[] {
		...
		for (const variant of product.variants) {
			...
			outputLineData.push(
				variantLineData.join(this.DELIMITER_) + this.NEWLINE_
			);
		}
